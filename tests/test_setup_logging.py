...
from sphinxcontrib_versioning.setup_logging import ColorFormatter, setup_logging
...
def test_arrow(tmpdir, verbose):
    ...
    :param int verbose: Verbosity level.
    """
    assert ColorFormatter.SPECIAL_SCOPE == "sphinxcontrib_versioning"
...
def test_colors(tmpdir):
    ...
    srcipt_path = tmpdir / "script.py"
    srcipt_path.write(script)
    output = pytest.run(tmpdir, [sys.executable, "script.py"])
