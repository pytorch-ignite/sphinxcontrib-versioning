...
REMOTES = (
    ...
    (
        "3b7987d8f5f50457f960cfbb04f69b4f1cb3e5ac",
        "v1.2.0",
        "tags",
        1433133463,
        "README",
    ...
    (
        "c4f19d2996ed1ab027b342dd0685157e3572679d",
        "v2.0.0",
        "tags",
        2444613111,
        "README",
    ...
    (
        "936956cca39e93cf727e056bfa631bb92319d197",
        "v2.1.0",
        "tags",
        1446526830,
        "README",
    ...
    (
        "23781ad05995212d3304fa5e97a37540c35f18a2",
        "v3.0.0",
        "tags",
        1464657292,
        "README",
    ...
    (
        "a0db52ded175520aa194e21c4b65b2095ad45358",
        "v10.0.0",
        "tags",
        1464657293,
        "README",
...
def test_no_sort(remotes):
    ...
    expected_branches = [
        (r[1], "../{}/contents.html".format(r[1])) for r in remotes if r[2] == "heads"
    ]
    expected_tags = [
        (r[1], "../{}/contents.html".format(r[1])) for r in remotes if r[2] == "tags"
    ]
...
@pytest.mark.parametrize(
    "sort", ["", "alpha", "time", "semver", "semver,alpha", "semver,time"]
)
def test_sort_valid(sort):
    ...
    versions.context.update(
        dict(pagename="contents", scv_is_root=True, current_version="main")
    )
...
def test_sort_semver_invalid(sort):
    ...
    if sort == "alpha":
        expected = ["a", "gh-pages", "main", "z"]
    elif sort == "time":
        expected = list(reversed(items))
    elif sort == "semver":
        expected = ["main", "gh-pages", "a", "z"]
    elif sort == "semver,alpha":
        expected = ["a", "gh-pages", "main", "z"]
    elif sort == "semver,time":
        expected = ["z", "a", "gh-pages", "main"]
    else:
        expected = items
...
@pytest.mark.parametrize("remotes", REMOTES_SHIFTED)
@pytest.mark.parametrize(
    "sort", ["alpha", "time", "semver", "semver,alpha", "semver,time", "invalid", ""]
)
