...
from sphinxcontrib_versioning.git import clone, GitError, IS_WINDOWS
...
def test_no_exclude(tmpdir, local_docs):
    ...
    clone(str(local_docs), str(new_root), "origin", "main", "", None)
    ...
    assert branch == "main"
    pytest.run(
        local_docs, ["git", "diff-index", "--quiet", "HEAD", "--"]
    )  # Exit 0 if nothing changed.
    pytest.run(
        new_root, ["git", "diff-index", "--quiet", "HEAD", "--"]
    )  # Exit 0 if nothing changed.
...
def test_exclude(tmpdir, local):
    ...
    pytest.run(local, ["git", "checkout", "main"])

    # Run.
    exclude = [
        ".travis.yml",
        "appveyor.yml",  # Ignored (nonexistent), show warnings.
        "README",
        "two.txt",
        join("sub", "four.txt"),  # Only leave these.
    ]
    new_root = tmpdir.ensure_dir("new_root")
    ...
    paths = sorted(
        f.relto(new_root)
        for f in new_root.visit()
        if new_root.join(".git") not in f.parts()
    )
...
def test_exclude_subdir(tmpdir, local):
    ...
    new_root = tmpdir.ensure_dir("new_root")
    clone(str(local), str(new_root), "origin", "main", "sub", ["three.txt"])
    paths = sorted(
        f.relto(new_root)
        for f in new_root.visit()
        if new_root.join(".git") not in f.parts()
...
def test_exclude_patterns(tmpdir, local):
    ...
    new_root = tmpdir.ensure_dir("new_root")
    clone(
        str(local), str(new_root), "origin", "main", ".", ["*.md", join("*", "*.md")]
    )
    paths = sorted(
        f.relto(new_root)
        for f in new_root.visit()
...
def test_bad_branch_rel_dest_exclude(tmpdir, local):
    ...
    with pytest.raises(GitError) as exc:
        clone(
            str(local),
            str(tmpdir.ensure_dir("new_root")),
            "origin",
            "unknown_branch",
            ".",
            None,
    ...
    assert (
        "Remote branch unknown_branch not found in upstream origin" in exc.value.output
    )
    ...
    with pytest.raises(GitError) as exc:
        clone(
            str(local),
            str(tmpdir.ensure_dir("new_root2")),
            "origin",
            "main",
            "..",
            ["README"],
    ...
    with pytest.raises(GitError) as exc:
        clone(
            str(local),
            str(tmpdir.ensure_dir("new_root3")),
            "origin",
            "main",
            "unknown",
            ["README"],
    ...
    with pytest.raises(GitError) as exc:
        clone(
            str(local),
            str(tmpdir.ensure_dir("new_root4")),
            "origin",
            "main",
            ".",
            None,
    ...
    if IS_WINDOWS:
        assert (
            "'{}' does not appear to be a git repository".format(
                local.join("does_not_exist")
            )
            in exc.value.output
        )
    else:
        assert (
            "repository '{}' does not exist".format(local.join("does_not_exist"))
            in exc.value.output
        )
...
def test_multiple_remotes(tmpdir, local, remote):
    ...
    pytest.run(
        local, ["git", "remote", "set-url", "--push", "origin", str(origin_push)]
    )
    ...
    pytest.run(
        local, ["git", "remote", "set-url", "--push", "origin2", str(origin2_push)]
    )

    new_root = tmpdir.ensure_dir("new_root")
    clone(str(local), str(new_root), "origin", "main", "", None)
