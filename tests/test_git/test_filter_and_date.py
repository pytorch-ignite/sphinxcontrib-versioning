...
from sphinxcontrib_versioning.git import filter_and_date, GitError, list_remote
...
def test_three_commits_multiple_paths(local):
    ...
    pytest.run(local, ["git", "checkout", "-b", "subdir", "main"])
...
def test_outdated_local(tmpdir, local, remote):
    ...
    pytest.run(local_ahead, ["git", "commit", "-am", "Changed main"])
    pytest.run(local_ahead, ["git", "checkout", "feature"])
    local_ahead.join("README").write("changed")
    pytest.run(local_ahead, ["git", "commit", "-am", "Changed feature"])
    pytest.run(local_ahead, ["git", "push", "origin", "main", "feature"])
    ...
    # Pull and retry.
    pytest.run(local, ["git", "pull", "origin", "main"])
    pytest.run(local, ["git", "checkout", "feature"])
    pytest.run(local, ["git", "pull", "origin", "feature"])
    dates = filter_and_date(str(local), ["README"], shas)
    assert (
        len(dates) == 3
    )  # Original SHA is the same for everything. Plus above two commits.
