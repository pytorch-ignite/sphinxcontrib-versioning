...
def test_no_exclude(local_docs_ghp, urls):
    ...
    # Run.
    output = pytest.run(
        local_docs_ghp, ["sphinx-versioning", "push", ".", "gh-pages", "."]
    )
    ...
    urls(
        local_docs_ghp.join("contents.html"),
        ['<li><a href="main/contents.html">main</a></li>'],
    )
    urls(
        local_docs_ghp.join("main", "contents.html"),
...
def test_exclude(local_docs_ghp, urls):
    ...
    output = pytest.run(
        local_docs_ghp,
        ["sphinx-versioning", "push", ".", "gh-pages", "documents", "-e", "keep.txt"],
    )
    assert "Traceback" not in output
...
def test_root_ref(local_docs_ghp):
    ...
    # Run.
    output = pytest.run(
        local_docs_ghp, ["sphinx-versioning", "-N", "push", "-t", ".", "gh-pages", "."]
    )
    assert "Traceback" not in output
    assert "Failed to push to remote repository." not in output
    ...
    for line in iter(proc.stdout.readline, b""):
        ...
        if line.strip() == b"=> Building docs...":
            if give_up or not caused:
                ...
                pytest.run(
                    local_other, ["git", "commit", "-am", "Cause race condition."]
                )
...
def test_different_push(tmpdir, local_docs_ghp, urls):
    ...
    pytest.run(
        local_docs_ghp, ["git", "remote", "set-url", "origin", "--push", remote2]
    )
    ...
    assert not local_docs_ghp.join("main").check()
...
def test_second_remote(tmpdir, local_docs_ghp, urls, remove):
    ...
    output = pytest.run(
        local_docs_ghp,
        ["sphinx-versioning", "push", ".", "gh-pages", ".", "-P", "remote2"],
    )
    assert "Traceback" not in output
    assert "Successfully pushed to remote repository." in output
    ...
    if remove:
        with pytest.raises(CalledProcessError) as exc:
            pytest.run(local_docs_ghp, ["git", "checkout", "origin/gh-pages"])
        assert (
            "origin/gh-pages' did not match any file(s) known to git."
            in exc.value.output
        )
    else:
        pytest.run(local_docs_ghp, ["git", "checkout", "origin/gh-pages"])
...
def test_error_clone_failure(local_docs):
    ...
    assert (
        "fatal: Remote branch gh-pages not found in upstream origin" in exc.value.output
    )
...
def test_error_build_failure(local_docs_ghp):
    ...
    with pytest.raises(CalledProcessError) as exc:
        pytest.run(
            local_docs_ghp, ["sphinx-versioning", "-L", "push", ".", "gh-pages", "."]
        )
    ...
    assert "sphinx-build failed for branch/tag: main" in exc.value.output
...
def test_bad_git_config(local_docs_ghp):
    ...
    env = dict(
        os.environ,
        GIT_DIR=str(local_docs_ghp.join(".git")),
        HOME=str(local_docs_ghp.join("..")),
    )
    ...
    for line in iter(proc.stdout.readline, b""):
        ...
        if b'"command": ["git", "clone"' in line:
            if not caused:
                ...
                tmp_repo = py.path.local(
                    re.findall(r'"cwd": "([^"]+)"', line.decode("utf-8"))[0]
                )
                assert tmp_repo.check(dir=True)
                pytest.run(
                    tmp_repo, ["git", "config", "user.useConfigOnly", "true"], retry=3
                )
    ...
    assert (
        "Please tell me who you are." in output
        or "user.useConfigOnly set but no name given" in output
    )
