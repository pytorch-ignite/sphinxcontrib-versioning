...
def commit_and_push(local_root, remote, versions):
    ...
    current_branch = run_command(
        local_root, ["git", "rev-parse", "--abbrev-ref", "HEAD"]
    ).strip()
    ...
    try:
        run_command(
            local_root,
            ["git", "diff", "HEAD", "--no-ext-diff", "--quiet", "--exit-code"],
        )
    except CalledProcessError:
    ...
    output = run_command(
        local_root, ["git", "diff", "HEAD", "--no-ext-diff", "--name-status"]
    )
