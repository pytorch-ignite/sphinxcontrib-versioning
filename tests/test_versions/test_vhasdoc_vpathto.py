...
def get_versions(context):
    ...
    """
    versions = Versions(
        [i * 5, i, "heads", 1465766422, "README"] for i in ("a", "b", "c")
    )
    versions.context.update(context)

    versions["a"]["found_docs"] = (
        "contents",
        "1",
        "sub/2",
        "sub/sub/3",
    ...
    versions["b"]["main_doc"] = "contents"
    versions["b"]["root_dir"] = "b"

    versions["c"]["found_docs"] = (
        "contents",
        "A",
        "sub/B",
        "sub/sub/C",
