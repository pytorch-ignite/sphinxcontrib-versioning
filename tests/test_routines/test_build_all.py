...
from sphinxcontrib_versioning.git import export
from sphinxcontrib_versioning.lib import HandledError
from sphinxcontrib_versioning.routines import build_all, gather_git_info
...
def test_single(tmpdir, local_docs, urls):
    ...
    export(
        str(local_docs),
        versions["main"]["sha"],
        str(exported_root.join(versions["main"]["sha"])),
    )
...
def test_multiple(tmpdir, config, local_docs, urls, triple, parallel):
    ...
    :param local_docs: conftest fixture.
    :param urls: conftest fixture.
    :param bool triple: With three versions (including main) instead of two.
    ...
    # Export (git tags point to same main sha).
    exported_root = tmpdir.ensure_dir("exported_root")
    export(
        str(local_docs),
        versions["main"]["sha"],
...
def test_banner_branch(tmpdir, banner, config, local_docs, show_banner):
    ...
    """
    pytest.run(local_docs, ["git", "checkout", "-b", "old_build", "main"])
    ...
    pytest.run(local_docs, ["git", "push", "origin", "main", "old_build"])

    config.show_banner = show_banner
    versions = Versions(gather_git_info(str(local_docs), ["conf.py"], tuple(), tuple()))
    versions["main"]["found_docs"] = ("contents", "one")
    ...
    # Verify banner.
    banner(
        dst.join("old_build", "contents.html"),
        "../main/contents.html",
        "the development version of Python. The main version is main",
    )
    banner(
        dst.join("old_build", "one.html"),
        "../main/one.html",
        "the development version of Python. The main version is main",
    )
...
def test_banner_tag(tmpdir, banner, config, local_docs, recent):
    ...
    pytest.run(local_docs, ["git", "push", "origin", "main", old, new])
    ...
    versions = Versions(gather_git_info(str(local_docs), ["conf.py"], tuple(), tuple()))
    versions["main"]["found_docs"] = ("contents", "one", "too")
    ...
    actual = sorted(
        ...
        for f in dst.visit(
            lambda p: p.basename
            in ("contents.html", "one.html", "two.html", "too.html")
        )
    )
    expected = [
        "contents.html",
    ...
    for page in ("contents", "one", "too"):
        ...
        banner(
            dst.join("main", "{}.html".format(page)),
            "../{new}/{page}.html".format(new=new, page=page),
            "the development version of Python. The latest version is {}".format(new),
        )
    ...
    banner(
        dst.join(old, "contents.html"),
        "../{}/contents.html".format(new),
        "an old version of Python. The latest version is {}".format(new),
    )
    banner(
        dst.join(old, "one.html"),
        "../{}/one.html".format(new),
        "an old version of Python. The latest version is {}".format(new),
    )
...
def test_all_errors(tmpdir, local_docs, urls):
    ...
    :param urls: conftest fixture.
    """
    pytest.run(local_docs, ["git", "checkout", "-b", "a_broken", "main"])
    local_docs.join("conf.py").write("main_doc = exception\n")
    pytest.run(local_docs, ["git", "commit", "-am", "Broken version."])
    ...
    destination = tmpdir.ensure_dir("destination")
    build_all(str(exported_root), str(destination), versions)
    assert [r["name"] for r in versions.remotes] == ["main"]
