...
from sphinxcontrib_versioning import __version__
from sphinxcontrib_versioning.git import clone, commit_and_push, get_root, GitError
from sphinxcontrib_versioning.lib import Config, HandledError, TempDir
from sphinxcontrib_versioning.routines import (
    build_all,
    gather_git_info,
    pre_build,
    read_local_conf,
)
from sphinxcontrib_versioning.setup_logging import setup_logging
from sphinxcontrib_versioning.versions import multi_sort, Versions
...
@click.option(
    "-l",
    "--local-conf",
    help="Path to conf.py for SCVersioning to read config from.",
    type=IS_EXISTS_FILE,
)
@click.option(
    "-L",
...
def cli(config, **options):
    """Build versioned Sphinx docs for every branch and tag.

    Supports only building locally with the "build" sub command. For more information for either run them with their own --help.

    The options below are global and must be specified before the sub command name (e.g. -N build ...).
    \f

    :param sphinxcontrib_versioning.lib.Config config: Runtime configuration.
